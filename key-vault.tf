locals { 
  ...
  managed_identities = concat(var.managed_identities, [
     {
        principal_name = azurerm_app_service.app_service.name // app name 
     }
  ])
}
...
module key_vault {
  ...
  private_endpoint_enabled = (
    contains(var.private_endpoint_resources_enabled, "keyVault") ? true : false
  )

  depends_on = [data.external.get_ase_subnet, azurerm_app_service.app_service]
}